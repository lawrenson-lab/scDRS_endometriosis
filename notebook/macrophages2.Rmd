...
sf2<-temp%>%ggplot(aes(x=cell_type,y=ID,fill=p.adjust,col=p.adjust,size=Count,shape=sign))+
  ...
  theme(axis.text.x = element_text(angle=45,vjust =1,hjust = 1,size = 13),
        legend.box = "vertical",axis.text = element_text(colour = "#000000"))+
  labs(x="",y="Pathway",shape="",col="P-val")
...
sf3<-temp%>%filter(pathway%in%c("NFkB","PI3K","TGFb","TNFa"))%>%
  ggplot(aes(x=activity,y=avg_score,group=group_major_plus_immune,color=group_major_plus_immune))+
  ...
  guides(col=guide_legend(nrow=2))+labs(col="",x="Activity",y="Average scDRS",size="Cell")
...
eqtl<-eqtl%>%
  ...
  mutate(Gene=factor(Gene,levels=i),
         ...
         Effect=ifelse(Risk_Allele_Effect=="pos","positive","negative"))
eqtl<-eqtl%>%mutate(Tissue=str_replace(Tissue,"eQTLGen cis eQTLs","Blood (eQTLGen)")%>%
                    str_replace("PsychENCODE eQTLs","Brain (PsychENCODE)")%>%
                    str_replace("TwinsUK ge LCL","Blood and lymphoblastoid (TwinsUK)")%>%
                    str_replace("TwinsUK ge skin","Skin (TwinsUK)"),
                    tissue_type=ifelse(str_detect(Tissue,"B-|Dendri|CD4|CD8|Monocy|Bloo|Sple"),
                                  "Immune","Non-immune"))

sf5<-eqtl%>%mutate(tissue_type=factor(tissue_type,levels=c("Non-immune","Immune")))%>%
  ggplot(aes(x=Gene,y=Tissue,shape=Effect,size=as.numeric(P)))+
    geom_point(fill="darkgray",col="darkgray")+scale_size_continuous(trans = "log10")+
    theme_classic(base_size = 14)+
    scale_shape_manual(values =25:24)+scale_size(range=c(6,1))+
    labs(y="Tissue",size="P-val",shape="Effect allele\nexpression")+
...
ggsave("supp_fig2.pdf",width = 10,height = 10,sup1)
