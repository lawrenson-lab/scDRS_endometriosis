...
sf8<-plot_df%>%filter(trait=="IBS")%>%
  ggplot(aes(y=group,x=total))+
  geom_bar(stat="identity")+
  theme_classic(base_size = 16)+
  theme(axis.text.x = element_text(angle=90))
p1<-plot_grid(get_panel(sf7),get_panel(sf8),rel_widths = c(.7,.3))
p2<-plot_grid(get_x_axis(sf7),get_x_axis(sf8),rel_widths = c(.7,.3))
p1<-plot_grid(p1,p2,nrow=2)
p2<-plot_grid(get_y_axis(sf7),NULL,nrow = 2)
p1<-plot_grid(p2,p1,ncol=2,rel_widths = c(.35,.65))
sf7<-plot_grid(p1,NULL,get_legend(sf7),nrow=3,rel_heights = c(.9,-.2,.1))
